...
// Tweaks by LSiwora (1.5)
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// LS - I remained with Matso's 3.0 DoF for I like it better than the latest version.
// You're welcome to upgrade it if you like.
...
/*------------------------------------------------------------------------------
				  ENB prepass modification 3.0.0 by Matso
...
#define CHROMA_POW		5.0								// the bigger the value, the more visible chomatic aberration effect in DoF
...
float fDofCutoff = 0.28;		// manages the smoothness of the DoF (bigger value results in wider depth of field)
float fDofBias = 0.07;			// distance not taken into account in DoF (all closer then the distance is in focus)
float fBlurScale = 0.003;		// governs image blur scale (the bigger value, the stronger blur)
float fBlurCutoff = 0.09;		// bluring tolerance depending on the pixel and sample depth (smaller causes objects edges to be preserved)

// Bokeh parameters
float fBokehCurve = 5.0;		// the larger the value, the more visible the bokeh effect is (not used with brightness limiting)
float fBokehIntensity = 0.95;		// governs bokeh brightness (not used with brightness limiting)
float fBokehConstant = 0.01;		// constant value of the bokeh weighting (bigger cause more powerful bokeh)
float fBokehMaxLevel = 850.0;		// bokeh max brightness level (scale factor for bokeh samples)
float fBokehMin = 0.001;		// min input cutoff (anything below is 0)
float fBokehMax = 1.985;		// max input cutoff (anything above is 1)

// Grain parameters
float fGrainFreq = 3000.0;						// movie grain frequency
float fGrainScale = 0.04;						// effect scale
...
{
	float4 res;
	float2 base = UnderWaterDistortion(IN.txcoord.xy);		// get distorted coordinates (should player is under water [WIP])
	float4 tcol = tex2D(SamplerColor, base.xy);
	float sd = tex2D(SamplerDepth, IN.txcoord.xy).x;		// acquire scene depth for the pixel
	...
	for (int i = 0; i < 4; i++)
	{
		...
		float4 ct = ChromaticAberration(coord.xy, outOfFocus);		// chromatic aberration sampling
#else
		...
		float b = GrayScale(ct.rgb) + length(ct.rgb) + fBokehConstant;
    #ifndef USE_BRIGHTNESS_LIMITING
		float w = pow(b * fBokehIntensity, fBokehCurve);
    #else
		float w = smoothstep(fBokehMin, fBokehMax, b) * fBokehMaxLevel;	// all samples above max input will be limited to max level
    #endif
		w += abs(offset[i]) + blur;
  #endif
#endif
		w *= (1.0 - smoothstep(0.0, fBlurCutoff, abs(sds - sd)));		// edges preserving
		tcol += ct * w;
		wValue += w;
	}
